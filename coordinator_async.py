...
def get_server(key, N):
    if N > len(servers):
        print(f"Number of actice server is momntanely {len(servers)}, so key will stored in all the servers")
        N = len(servers)
...
def writing_on_servers(server_su_cui_scrivere, d):
    ...
    if success_count == N:
        print({'message': 'Wrote on servers', 'key': d['key'], 'value': d['value']})
    else:
        print({'error': 'Failed to write on each server'})
...
def put(N): 
    ...
    scrittura = threading.Thread(target=writing_on_servers, args=(server_su_cui_scrivere, d))
    scrittura.start()
    return jsonify({'message': 'Writing on servers...', 'key': key, 'value': d['value']}), 200
    ...
    while True:
        ...
        for server in active_servers:
            if active_servers[server] == 0 and server in servers:
                ...
                # aggiornare il server che è tornato attivo con i valori degli altri server
                update(server)
                # il server torna attivo e lo aggiungo alla lista dei server attivi
